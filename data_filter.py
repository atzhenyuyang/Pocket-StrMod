...
ALERT_STRUCTURES = [Chem.MolFromSmarts(alert_smarts) for alert_smarts in\
...
def has_alert_struct(mol):
    #m = Chem.MolFromSmiles(Chem.MolToSmiles(mol, isomericSmiles=False))
    Chem.GetSSSR(mol)
    for p in ALERT_STRUCTURES:
...
def verify_dir_exists(dirname):
    if os.path.isdir(os.path.dirname(dirname)) == False:
        try:
            ...
            print("FileExistsError: [Errno 17] File may exists: '{}'".format(dirname))
...
def process(gz_file):
    ...
    try:
        ...
        valid_mol_list = []
        with gzip.open(gz_file) as f_gz:
        #for f_gz in [gz_file]:
            mols = Chem.ForwardSDMolSupplier(f_gz)
            #mols = Chem.SDMolSupplier(f_gz)
            for m in mols:
                if m is None: continue
                has_8_member_ring = False
                molwt = Descriptors.MolWt(m)
                if molwt < 300 or molwt > 600: continue
                if QED.qed(m) < 0.7: continue
                if check_lipinski(m) == False: continue
                ring_info = m.GetRingInfo()
                if Chem.GetSSSR(m) > 5: continue
                num_N = 0
                num_O = 0
                for ix, a in enumerate(m.GetAtoms()):
                    symbol = a.GetSymbol()
                    if symbol == 'N': num_N += 1
                    if symbol == 'O': num_O += 1
                    if symbol not in elements:
                        ...
                        ele = True
                    if ring_info.MinAtomRingSize(ix) > 7:
                        has_8_member_ring = True
                        break
                if ele == False: continue
                if has_8_member_ring: continue
                if num_N < 1: continue
                if num_O < 1: continue
                m_ = Chem.MolFromSmiles(Chem.MolToSmiles(m, isomericSmiles=False))
                if has_fused4ring(m_): continue
                if has_alert_struct(m_): continue
                valid_mol_list.append(Chem.MolToMolBlock(m))
                n += 1
        if valid_mol_list:
            save_dir = 'MolWt300-600_QED0.7_N1_O1_MinRing7_SSSR5_Lipinski_Alert_FusedRing/' + '/'.join(gz_file.split('/')[-3:-1]) + '/'
            verify_dir_exists(save_dir)
            save_name = '.'.join(gz_file.split('/')[-1].split('.')[:-1])
            with open(save_dir + '/' + save_name, 'w') as fw:
                fw.write('\n$$$$\n'.join(valid_mol_list))
            os.system('gzip {}'.format(save_dir + '/' + save_name))
        return n
    except:
        print(gz_file)
        return n
...
path = './ZINC/*/*/*sdf*'
gz_files = glob.glob(path)
pool = Pool(processes=20)                 
data_pool = pool.map(process, gz_files)  
pool.close()
pool.join()
print('Total Num: ', sum(data_pool))
