...
ALERT_STRUCTURES = [
    ...
    'C=N([C,S])[C&v4]', 'N=[C!R]=O', 'N=[C!R]=S', 'C=[C!R]=O', 'P(=S)(S)S', 
...
EXCLUDE_STRUCTURES = []
# examples
"""EXCLUDE_STRUCTURES = ['[#8]=[#6]1~&@[#7](-[#6](~[#6,#7,#8,#9,#15,#16,#17,#35,#53])~[#6])~&@[#6](~&@[#6](~&@[#7]~&@[#7]~&@1-[#6]1~&@[#6]~&@[#6]~&@[#6]~&@[#6]~&@[#6]~&@1)-[#6]#[#7])=[#8]',
                      '[OH]',
                      '[NH2]']"""
...
def check_exclude_struct(mol):
    for temp in EXCLUDE_STRUCTURES:
        p = Chem.MolFromSmarts(temp)
        if mol.HasSubstructMatch(p):
            return False
    else:
        return True
...
class Filter(object):
    def __init__(self, sa_threshhold=5.0, qed_threshhold=0.3, mw_threshhold=(300,700)):
    ...
    def _filter(self, mol):
        try:
            ...
            mw_check = self.mw_threshhold[0] < Descriptors.MolWt(mol) < self.mw_threshhold[1]
            lip_check = check_lipinski(mol)
            if qed_check and sa_check and mw_check and lip_check:
                struct_check = check_alert_struct(mol)
                exclude_check = check_exclude_struct(mol)
                if struct_check and exclude_check:
    ...
    def run(self, mol_dict, save_name='Filter_Result.sdf', mol_name=None):
        num_mol = 0
        smi_dict = {}
        for k,v in tqdm(mol_dict.items()):
            m = Chem.MolFromMolBlock(v)
